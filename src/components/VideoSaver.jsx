...
export default function CGHMHealthDashboard() {
  ...
  const initCamera = async () => {
    ...
    try {
      ...
      if (videoRef.current) {
        ...
        // --- SAMSUNG FIX START ---
        // Safely get capabilities. If getCapabilities is missing, provide empty object
        const capabilities = videoTrackRef.current.getCapabilities ? videoTrackRef.current.getCapabilities() : {};
        console.log("Device Capabilities:", capabilities);

        // Force 'hasFlash' to true to bypass Samsung's false reporting. 
        // We will handle actual hardware rejection inside toggleFlash.
        setHasFlash(true);
        // --- SAMSUNG FIX END ---
  ...
  const stopCamera = useCallback(() => {
    ...
    if (
      (appStateRef.current === "RECORDING" || appStateRef.current === "RECORDING_WARNING") &&
  ...
  const toggleFlash = async (turnOn) => {
    ...
    try {
      // Apply constraints directly to force the hardware
      await videoTrackRef.current.applyConstraints({ advanced: [{ torch: turnOn }] });
      setIsFlashOn(turnOn);
    } catch (e) {
      console.error(`Flash Error:`, e);
      // Fallback: If Samsung strictly blocks it, notify the user gracefully
      toast.error(`Flashlight error: Device cannot activate torch (${e.name})`);
      setIsFlashOn(false);
    }
  };
      ...
      <div className="bg-white p-8 rounded-2xl shadow-xl w-11/12 max-w-2xl flex flex-col items-center relative">
        ...
        <div className="flex flex-wrap gap-4 w-full">
          {uiAppState === "IDLE" && (
            <button
              onClick={() => {
                setSubjectData({ id: "", name: "", age: "", gender: "", mobile: "", height: "" });
          ...
          {uiAppState === "WAITING_START" && (
            <button
              onClick={startRecordingLogic}
              className="flex-1 py-4 px-6 rounded-lg font-bold text-white text-lg shadow-lg bg-[#12863B] animate-bounce hover:opacity-90"
            >
              ▶ Start Recording
            </button>
          )}
          ...
          {uiAppState === "RECORDING" && (
            <button
              onClick={stopRecordingSuccess}
              className="flex-1 py-4 px-6 rounded-lg font-bold text-white text-lg shadow-lg bg-[#05205A] hover:bg-[#041642] transition-colors"
            >
              ⏹ Stop Recording (Save)
            </button>
          )}
          ...
          {uiAppState !== "IDLE" && uiAppState !== "MODAL_OPEN" && (
            <button
              onClick={stopCamera}
              disabled={uploading}
              className={`py-4 px-6 rounded-lg font-bold text-white text-lg shadow-lg bg-[#CC161C] transition-opacity ${uploading ? "opacity-50 cursor-not-allowed" : "hover:opacity-90"}`}
            >
              Cancel
            </button>
          )}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all">
            ...
            >
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setSubjectData({ ...subjectData, id: e.target.value })}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setSubjectData({ ...subjectData, age: e.target.value })}
                    className="w-full border-2 border-gray-200 rounded-lg p-2 focus:border-[#05205A] outline-none"
                  />
                </div>
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setSubjectData({ ...subjectData, gender: e.target.value })}
                    className="w-full border-2 border-gray-200 rounded-lg p-2 focus:border-[#05205A] outline-none"
                  >
              ...
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1">Mobile (Optional)</label>
                <input
                  type="tel"
                  value={subjectData.mobile}
                  onChange={(e) => setSubjectData({ ...subjectData, mobile: e.target.value })}
                  className="w-full border-2 border-gray-200 rounded-lg p-2 focus:border-[#05205A] outline-none"
                />
              </div>
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1">Height cm (Optional)</label>
                <input
                  type="number"
                  value={subjectData.height}
                  onChange={(e) => setSubjectData({ ...subjectData, height: e.target.value })}
                  className="w-full border-2 border-gray-200 rounded-lg p-2 focus:border-[#05205A] outline-none"
                />
              </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden border-t-8 border-[#12863B] transform transition-all">
            <div className="p-6 text-center">
              ...
              <p className="text-sm text-gray-500 mb-6">
                Apni parameters niche enter karein. Location aapko next step me puchi jayegi.
              </p>
              <form
                onSubmit={(e) => {
                  e.preventDefault();
              ...
              >
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-bold text-gray-700 mb-1">Systolic BP</label>
                    <input
                      type="text"
                      placeholder="120"
                      value={referenceData.sys}
                      onChange={(e) => setReferenceData({ ...referenceData, sys: e.target.value })}
                      className="w-full border-2 border-gray-200 rounded-lg p-2 focus:border-[#05205A] outline-none"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-gray-700 mb-1">Diastolic BP</label>
                    <input
                      type="text"
                      placeholder="80"
                      value={referenceData.dia}
                      onChange={(e) => setReferenceData({ ...referenceData, dia: e.target.value })}
                      className="w-full border-2 border-gray-200 rounded-lg p-2 focus:border-[#05205A] outline-none"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-bold text-gray-700 mb-1">Pulse Rate</label>
                    <input
                      type="text"
                      placeholder="72"
                      value={referenceData.pulse}
                      onChange={(e) => setReferenceData({ ...referenceData, pulse: e.target.value })}
                      className="w-full border-2 border-gray-200 rounded-lg p-2 focus:border-[#05205A] outline-none"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-gray-700 mb-1">Sugar Level</label>
                    <input
                      type="text"
                      placeholder="110"
                      value={referenceData.sugar}
                      onChange={(e) => setReferenceData({ ...referenceData, sugar: e.target.value })}
                      className="w-full border-2 border-gray-200 rounded-lg p-2 focus:border-[#05205A] outline-none bg-green-50"
                    />
                  </div>
                </div>
                ...
                <div className="flex gap-3 pt-6 sticky bottom-0 bg-white z-10">
                  <button
                    type="button"
                    disabled={uploading}
                    onClick={() => saveAndDownloadFiles({ sys: "", dia: "", pulse: "", sugar: "" })}
                    className="flex-1 py-3 px-4 border-2 border-gray-300 rounded-lg font-bold hover:bg-gray-50 disabled:opacity-50 text-sm"
                  >
                    Skip & Select Location
                  </button>
                  <button
                    type="submit"
                    disabled={uploading}
                    className="flex-2 py-3 px-4 bg-[#05205A] text-white rounded-lg font-bold hover:opacity-90 flex items-center justify-center disabled:opacity-50"
                  >
                    Save Data
                  </button>
                </div>
